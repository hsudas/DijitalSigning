    ...
    public class ImageController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                ...
                if (Request.Form["txtImageName"] != null)
                {
                    pagestate.ImageName = Request.Form["txtImageName"].ToString().Trim();
                }
                if (Request.Form["txtTag"] != null)
                {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    else
                    {
                        ...
                        {
                            ViewData["ValidationMessage"] = "Dosya kopyalanamadı.";
                            ViewData["FileList"] = new SelectList(BuildFileList(Request.Form["lstFile"].ToString().Trim()), "Value", "Text", image.OriginalFilename);
                            ViewData["ImageURL"] = selectedfile;
                            return View(image);
                        }
                        ...
                        CommonMethods.CreateActivityLog((User)Session["User"], "Image", "Add",
                                "Resim eklendi '" + image.ImageName + "' - ID: " + image.ImageID.ToString());
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    CommonMethods.CreateActivityLog((User)Session["User"], "Image", "Edit",
                            "Resim güncellendi '" + image.ImageName + "' - ID: " + image.ImageID.ToString());
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (file != null && file.ContentLength > 0 && !String.IsNullOrEmpty(file.FileName))
                    {
                        ...
                        {
                            ViewData["ValidationMessage"] = "Yüklenen dosyalar 200MB veya daha az olmalıdır.";
                        }
                        else
                        {
                            ...
                            {
                                ViewData["ValidationMessage"] = "Sadece .jpg, .jpeg, and .png uzantılı dosyalar upload edilebilir.";
                            }
                            else
                            {
                    ...
                    {
                        ViewData["ValidationMessage"] = "Bir dosyayı seçmelisiniz.";
                        return View(image);
                    }
                    ...
                    else
                    {
                        ...
                        CommonMethods.CreateActivityLog((User)Session["User"], "Image", "Upload",
                                "Resim yüklendi '" + image.ImageName + "' - ID: " + image.ImageID.ToString());
        ...
        private string ValidateInput(Image image)
        {
            ...
            if (image.OriginalFilename == "0" || String.IsNullOrEmpty(image.OriginalFilename))
                return "Geçerli bir resim seçmelisiniz.";

            if (String.IsNullOrEmpty(image.ImageName))
                return "Resim Adı zorunludur.";

            if (String.IsNullOrEmpty(image.OriginalFilename))
                return "Orjinal Dosya Adı zorunludur.";

            if (String.IsNullOrEmpty(image.StoredFilename))
                return "Stored Filename zorunludur.";
